from pydantic import BaseModel, Field


class ConversationMessage(BaseModel):
    type: str
    text: str
    utterance_id: str | None = None
    badge_type: str | None = None
    speaker: str | None = None


class SessionState(BaseModel):
    session_id: str
    current_segments: list[tuple[str, str | None]] = Field(default_factory=list)
    messages: list[ConversationMessage] = Field(default_factory=list)
    extracted_fields: dict[str, str] = Field(default_factory=dict)
    customer_last_utterance: str = Field(default="")
    agent_last_utterance: str = Field(default="")
    call_stage: str = Field(default="unknown")
    sentiment: str = Field(default="neutral")
    rolling_summary: str = Field(default="")
    customer_history: list[str] = Field(default_factory=list)
    agent_history: list[str] = Field(default_factory=list)
    lead_id: str | None = Field(default=None)
    lead_detail: dict = Field(default_factory=dict)
    lead_facts: dict = Field(default_factory=dict)
    lead_missing_fields: list[dict] = Field(default_factory=list)
    lead_priority_missing_fields: list[dict] = Field(default_factory=list)
    resolved_field_state: dict = Field(default_factory=dict)
    active_category: str | None = Field(default=None)
    category_route: dict = Field(default_factory=dict)
    workflow_state: dict = Field(default_factory=dict)
    last_next_action: dict = Field(default_factory=dict)
    last_suggestion: str = Field(default="")  # last AI suggestion — used to prevent repetition
    last_triggered_utterance_key: str = Field(default="")
    last_triggered_utterance_at: float = Field(default=0.0)
