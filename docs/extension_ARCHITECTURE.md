...
The Gemini Chrome Extension is designed to provide low-latency, real-time audio transcription and AI-driven analysis. It leverages a modern Manifest V3 architecture with a multi-process approach.
...
### Components:
- **Background Script (`background.js`)**: Orchestrates the entire system. It manages the extension's state, coordinates communication between components, and handles the streaming interaction with the Gemini AI.
...
## 3. AI Analysis Pipeline (Gemini Streaming)
...
- The Background Script calls the **Gemini Pro API**'s `streamGenerateContent` endpoint.
